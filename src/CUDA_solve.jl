struct BlockTriDiagData_CUDA{
    ...
    NextData::Union{BlockTriDiagData_CUDA, Nothing}
...
function initialize_CUDA(N, m, n, P, A_list_final, B_list_final, level)
    ...
    for i = 1:level
        ...
        LHS_A_list = CUDA.similar(A_list_final, n, n, P);
        LHS_B_list = CUDA.similar(A_list_final, n, n, P-1);

        RHS = CUDA.similar(A_list_final, P * n);

        MA_chol_A_list = CUDA.similar(A_list_final, n, n, m, P-1);
        MA_chol_B_list = CUDA.similar(A_list_final, n, n, m-1, P-1);
    
        LHS_chol_A_list = CUDA.similar(A_list_final, n, n, P);
        LHS_chol_B_list = CUDA.similar(A_list_final, n, n, P-1);

        factor_list = CUDA.similar(A_list_final, m*n, 2*n, P-1);


        M_2n = CUDA.similar(A_list_final, 2*n, 2*n);
        M_mn_2n_1 = CUDA.similar(A_list_final, m*n, 2*n);
        M_mn_2n_2 = CUDA.similar(A_list_final, m*n, 2*n);
        ...
        next_x = CUDA.similar(A_list_final, P*n);

        if i == level
            A_list = A_list_final
            B_list = B_list_final
        else
            A_list = CUDA.similar(A_list_final, n, n, N);
            B_list = CUDA.similar(A_list_final, n, n, N-1);
        end

        data = BlockTriDiagData_CUDA(
            N, 
            m, 
            n, 
            P, 
...
if isnothing(data.NextData)
    ...
    factorize_CUDA!(data.NextData)
end

end

function solve_CUDA!(data::BlockTriDiagData_CUDA, d, x)
    P = data.P
    n = data.n
    m = data.m
    ...
    if isnothing(data.NextData)
        ...
        solve_CUDA!(data.NextData, RHS, data.next_x)
