@testset "Full cholesky factor" begin
    ...
    for run in 1:3
        ...
        data = initialize_full_cholesky_factor(N, m, n, P, A_list, B_list, 3);
        x = zeros(data.N * n);

        GC.gc()
        println("  Factorization:")
        @time factorize_full_cholesky_factor!(data)
        
        println("  Solve:")
        @time solve_full_cholesky_factor!(data, d, x)
